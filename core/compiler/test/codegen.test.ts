...
// --- Test: AI_SERVICE KEYS_ENTITY mode ---
section('AI_SERVICE KEYS_ENTITY codegen');

const keysEntitySource = `APP ketest {
  TITLE  "Keys Entity Test"
  DB     ke.db
  PORT   5199
  THEME  dark
}
...
`;
const keResult = compile(keysEntitySource);
assert(keResult.files.has('src-tauri/src/ai_service.rs'), 'KEYS_ENTITY: should emit ai_service.rs');
const keAiRs = keResult.files.get('src-tauri/src/ai_service.rs')!;

// Must NOT emit file-based key helpers
assert(!keAiRs.includes('pub type ApiKeyStore'), 'KEYS_ENTITY: must NOT declare ApiKeyStore');
assert(!keAiRs.includes('pub fn get_api_keys'), 'KEYS_ENTITY: must NOT emit get_api_keys command');
assert(!keAiRs.includes('pub fn set_api_key'), 'KEYS_ENTITY: must NOT emit set_api_key command');
assert(!keAiRs.includes('pub fn save_api_keys'), 'KEYS_ENTITY: must NOT emit save_api_keys');
assert(!keAiRs.includes('api_keys_path()'), 'KEYS_ENTITY: must NOT call api_keys_path()');

// Must emit DB-based key helper
assert(keAiRs.includes('pub fn load_api_keys(conn: &rusqlite::Connection)'), 'KEYS_ENTITY: load_api_keys must accept a DB connection');
assert(keAiRs.includes("FROM ai_configs WHERE id = 'singleton'"), 'KEYS_ENTITY: must SELECT from ai_configs singleton row');
assert(keAiRs.includes('anthropic_api_key'), 'KEYS_ENTITY: must SELECT anthropic_api_key column');
assert(keAiRs.includes('openai_api_key'), 'KEYS_ENTITY: must SELECT openai_api_key column');

// send_chat must use DbPool, not ApiKeyStore
assert(keAiRs.includes("db: tauri::State<'_, DbPool>"), 'KEYS_ENTITY: send_chat must take DbPool state');
assert(keAiRs.includes('use crate::db::DbPool'), 'KEYS_ENTITY: must import DbPool from crate::db');

// discover_models must also use DbPool
assert(keAiRs.includes('pub async fn discover_models'), 'KEYS_ENTITY: should still expose discover_models');

// Shared functionality must still be present
assert(keAiRs.includes('pub async fn send_chat'), 'KEYS_ENTITY: should still expose send_chat');
assert(keAiRs.includes('async fn call_anthropic'), 'KEYS_ENTITY: should still emit call_anthropic');
assert(keAiRs.includes('async fn call_openai'), 'KEYS_ENTITY: should still emit call_openai');
assert(keAiRs.includes('pub struct ModelInfo'), 'KEYS_ENTITY: should still declare ModelInfo');
assert(keAiRs.includes('Generated by Agicore'), 'KEYS_ENTITY: should mark file as generated');
console.log(`  ai_service.rs (KEYS_ENTITY): ${keAiRs.length} bytes (${keAiRs.split('\n').length} lines)`);

// main.rs must NOT manage api_keys state in KEYS_ENTITY mode
const keMain = keResult.files.get('src-tauri/src/main.rs')!;
assert(!keMain.includes('api_keys = Mutex::new(ai_service::load_api_keys())'), 'KEYS_ENTITY: main.rs must not manage ApiKeyStore state');
assert(!keMain.includes('ai_service::get_api_keys'), 'KEYS_ENTITY: main.rs invoke_handler must not include get_api_keys');
assert(!keMain.includes('ai_service::set_api_key'), 'KEYS_ENTITY: main.rs invoke_handler must not include set_api_key');
assert(keMain.includes('ai_service::send_chat'), 'KEYS_ENTITY: main.rs must still register send_chat');
assert(keMain.includes('ai_service::discover_models'), 'KEYS_ENTITY: main.rs must still register discover_models');
...
assert(workspaceRs!.includes('DbPath'), 'switch_db should access DbPath to track current path');
assert(workspaceRs!.includes('new_conn.execute_batch(migration)'), 'switch_db should run migrations on new DB');
...
assert(wsDbRs!.includes('pub type DbPath = Mutex<PathBuf>;'), 'db.rs should export DbPath type alias');
...
const wsApiTs = wsFiles.get('src/lib/api.ts');
assert(wsApiTs!.includes("export const getDbPath"), 'api.ts should export getDbPath');
assert(wsApiTs!.includes("export const switchDb"), 'api.ts should export switchDb');
assert(wsApiTs!.includes("invoke<string>('get_db_path')"), 'getDbPath should invoke get_db_path');
assert(wsApiTs!.includes("invoke<void>('switch_db'"), 'switchDb should invoke switch_db');
