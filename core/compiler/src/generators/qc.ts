// QC Code Generator
// Generates src/lib/qc.ts — standalone SPC quality tracker from QC declarations.
// Persists stats to localStorage; no pipeline/orchestration coupling.
// Frontend components import { qc } from './qc' and call shouldSample() / recordResult().

import type { AgiFile, QCDecl } from '@agicore/parser';

function buildQcModule(qcs: QCDecl[]): string {
  const lines: string[] = [
...
export function generateQc(ast: AgiFile): Map<string, string> {
  const files = new Map<string, string>();
  if (ast.qcs.length === 0) return files;
  files.set('src/lib/qc.ts', buildQcModule(ast.qcs));
  return files;
}
