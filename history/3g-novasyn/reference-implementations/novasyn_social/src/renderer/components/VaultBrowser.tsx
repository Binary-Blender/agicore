...
export function VaultBrowser({ isOpen, onClose, onImport }: Props) {
  ...
  const handleAddAnnotation = async () => {
    ...
    try {
      ...
      setAnnotations(annots);
      loadItems(); // Refresh annotation counts
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-slate-800 border border-slate-600 rounded-xl w-[900px] h-[600px] shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-700 bg-slate-800/50">
          ...
          <button onClick={loadItems} className="bg-blue-600 hover:bg-blue-500 text-white text-sm px-3 py-1.5 rounded">
            Search
          </button>
        </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="space-y-1">
                      {provenance.map((p, i) => (
                        <div
                          key={p.id}
                          className={`flex items-center gap-2 text-xs ${p.id === selectedItem.id ? 'text-blue-400' : 'text-gray-400'}`}
                        >
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-2">
                    <input
                      ...
                      className="flex-1 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500"
                    />
                    <button
