...
const APP_ID = 'novasyn-code';
const APP_DISPLAY_NAME = 'NovaSyn Code';
...
function getCodeMacros(): Record<string, MacroDefinition> {
  return {
    'code.apply_file': {
      description: 'Apply code content to a file path',
      input: { filePath: 'string', content: 'string' },
      output: { success: 'boolean' },
    },
    'code.read_file': {
      description: 'Read file content from a project',
      input: { filePath: 'string' },
      output: { content: 'string' },
    },
    'code.send_prompt': {
      description: 'Send a coding prompt and get a response',
      input: { prompt: 'string', model: 'string?' },
      output: { response: 'string', model: 'string', tokens: 'number' },
    },
  };
}
