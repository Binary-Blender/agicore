...
const APP_ID = 'novasyn-orchestrator';
const APP_DISPLAY_NAME = 'NovaSyn Orchestrator';
...
function getOrchestratorMacros(): Record<string, MacroDefinition> {
  return {
    'orchestrator.run_workflow': {
      description: 'Execute a saved workflow by ID',
      input: { workflowId: 'string', inputs: 'object?' },
      output: { runId: 'string', status: 'string', results: 'array' },
    },
    'orchestrator.list_workflows': {
      description: 'List all saved workflows',
      input: { includeTemplates: 'boolean?' },
      output: { workflows: 'array' },
    },
  };
}
