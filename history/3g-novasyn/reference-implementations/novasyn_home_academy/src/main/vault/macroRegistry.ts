...
const APP_ID = 'novasyn-academy';
const APP_DISPLAY_NAME = 'NovaSyn Academy';
...
function getAcademyMacros(): Record<string, MacroDefinition> {
  return {
    'academy.generate_lesson': {
      description: 'Generate a lesson plan',
      input: { subject: 'string', gradeLevel: 'string', topic: 'string' },
      output: { lesson: 'object' },
    },
    'academy.create_quiz': {
      description: 'Create a quiz',
      input: { subject: 'string', topic: 'string', questionCount: 'number?' },
      output: { quiz: 'object' },
    },
  };
}
