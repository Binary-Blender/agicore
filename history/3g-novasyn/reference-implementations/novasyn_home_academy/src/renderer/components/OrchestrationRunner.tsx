...
import type { OrchestrationRun, StepResult, OrchestrationStep, Orchestration } from '../../shared/types';
...
export function OrchestrationRunner({ runId, orchestrationName, onClose }: Props) {
  ...
  // Load orchestration details (for step configs)
  ...
  // Register real-time progress listener + polling fallback
  useEffect(() => {
    fetchRun();

    // Real-time listener
    const unsubscribe = window.electronAPI.onOrchStepProgress((data) => {
      if (data.runId !== runId) return;
      // Update step result in place
      setRun((prev) => {
        if (!prev) return prev;
        ...
        // Derive run status from step statuses
  ...
  // Get previous step output for QC display
      ...
      <div className="bg-slate-800 border border-slate-600 rounded-xl w-[700px] h-[650px] shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {loading ? (
            ...
            (steps.length > 0 ? steps : Array.from({ length: stepResults.length }, (_, i) => null)).map((step, index) => {
              ...
              // Card container classes by status
              let cardClass = 'rounded-lg border p-3 transition-all ';
              switch (status) {
                case 'pending':
                  cardClass += 'bg-slate-700 border-slate-600 opacity-60';
                  break;
                case 'running':
                  cardClass += 'bg-blue-900/30 border-blue-500 orch-pulse';
                  break;
                case 'completed':
                  cardClass += 'bg-green-900/20 border-green-600';
                  break;
                case 'failed':
                  cardClass += 'bg-red-900/20 border-red-500';
                  break;
                case 'awaiting_qc':
                  cardClass += 'bg-yellow-900/20 border-yellow-500';
                  break;
                case 'skipped':
              ...
              return (
                <div key={step?.id || `step-${index}`} className={cardClass}>
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {status === 'running' && (
                      <div className="w-4 h-4 rounded-full bg-blue-500 flex-shrink-0 animate-pulse" />
                    )}
                    {status === 'completed' && (
                    ...
                    {/* Right side: status text / elapsed / latency */}
                  ...
                  {/* Status-specific detail content */}
                  ...
                  {status === 'awaiting_qc' && (
                    <div className="mt-3 space-y-3">
                      ...
                      {/* Previous step output for review */}
        ...
        <div className="px-4 py-2.5 border-t border-slate-700 flex items-center justify-between text-xs text-gray-400">
          <div className="flex items-center gap-4">
            ...
            <span>
              Steps: <span className="text-gray-200">{stepsCompleted}/{totalSteps}</span>
            </span>
            <span>
