...
async function executeAiAction(step: OrchestrationStep, input: string): Promise<string> {
  const prompt = step.config.promptTemplate
    ? step.config.promptTemplate.replace(/\{\{input\}\}/g, input)
    : input;

  const modelId = step.config.model || AVAILABLE_MODELS[0]?.id || 'claude-haiku-4-5-20251001';
  ...
  try {
    // Use aiService.streamCompletion but collect the full output
    let fullText = '';
    ...
    throw new Error(`AI action failed: ${err.message}`);
  }
}
