...
const APP_ID = 'novasyn-writer';
const APP_DISPLAY_NAME = 'NovaSyn Writer';
...
function getWriterMacros(): Record<string, MacroDefinition> {
  return {
    'writer.ai_rewrite': {
    ...
    'writer.export_chapter': {
      description: 'Export a chapter',
      input: { projectId: 'string', chapterId: 'string', format: 'string?' },
      output: { content: 'string', filePath: 'string?' },
    },
    'writer.generate_outline': {
      description: 'Generate a chapter outline',
      input: { topic: 'string', chapters: 'number?' },
