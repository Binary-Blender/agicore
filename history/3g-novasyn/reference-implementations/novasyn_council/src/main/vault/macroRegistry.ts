...
const APP_ID = 'novasyn-council';
const APP_DISPLAY_NAME = 'NovaSyn Council';
...
function getCouncilMacros(): Record<string, MacroDefinition> {
  return {
    'council.run_discussion': {
      description: 'Run a multi-persona discussion',
      input: { topic: 'string', personaIds: 'array?', rounds: 'number?' },
      output: { summary: 'string', meetingId: 'string' },
    },
    'council.get_consensus': {
      description: 'Get consensus from a meeting',
      input: { meetingId: 'string' },
      output: { consensus: 'string', dissent: 'string?' },
    },
  };
}
