...
// Vault: shared asset storage at %APPDATA%/NovaSyn/vault.db
...
pub fn vault_search_assets(vault: tauri::State<'_, VaultPool>, query: String, asset_type: Option<String>) -> Result<Vec<VaultAsset>, String> {
    ...
    let rows = if let Some(t) = asset_type {
        ...
        // Bind to a local so the temporary borrowing stmt is dropped before stmt is.
        let v: Vec<VaultAsset> = stmt.query_map(rusqlite::params![t, like, like], |row| Ok(VaultAsset::from_row(row)))
            .map_err(|e| e.to_string())?
            .collect::<Result<Vec<_>, _>>()
            .map_err(|e| e.to_string())?;
        v
        ...
        let v: Vec<VaultAsset> = stmt.query_map(rusqlite::params![like, like], |row| Ok(VaultAsset::from_row(row)))
            .map_err(|e| e.to_string())?
            .collect::<Result<Vec<_>, _>>()
            .map_err(|e| e.to_string())?;
        v
