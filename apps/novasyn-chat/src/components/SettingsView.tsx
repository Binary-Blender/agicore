    ...
    <div className="flex-1 overflow-y-auto p-6 max-w-3xl mx-auto w-full">
            ...
            return (
              <div key={p.id} className="bg-slate-800/60 border border-slate-700 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <a href={p.url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-400 hover:text-blue-300 transition">
                    Get key →
                  </a>
                </div>
                ...
                {isEditing ? (
                  <div className="flex gap-2">
                    <input
                      ...
                      autoFocus
                    />
                    <button onClick={() => handleSave(p.id)} className="text-sm bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded transition">
                      Save
                    </button>
                    <button onClick={() => setEditing(null)} className="text-sm text-gray-400 hover:text-white px-3 py-1.5 rounded transition">
                      Cancel
                    </button>
                  </div>
                ) : (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="text-xs bg-slate-700 hover:bg-slate-600 text-gray-300 hover:text-white px-3 py-1.5 rounded transition"
                    >
                    ...
                    {hasKey && (
                      <button
                        ...
                        className="text-xs text-red-400 hover:text-red-300 px-2 py-1.5 rounded transition"
                        title="Remove"
                      >
                        Remove
                      </button>
      ...
      <section className="mb-8">
        ...
        <div className="bg-slate-800/60 border border-slate-700 rounded-lg p-4 space-y-2 text-sm text-gray-300">
          <p><span className="text-gray-500">Location:</span> <code className="bg-slate-900/50 px-1.5 py-0.5 rounded text-xs">%APPDATA%/NovaSyn/novasyn_chat.db</code></p>
          <p className="text-xs text-gray-500">Schema-compatible with NovaSyn Chat 1.x. Both versions can read/write the same database.</p>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="bg-slate-800/60 border border-slate-700 rounded-lg p-4 text-sm text-gray-300 space-y-1">
          <p><span className="text-gray-500">Version:</span> 2.0.0</p>
          <p><span className="text-gray-500">Framework:</span> Agicore DSL (31 declaration types, 7 layers)</p>
          <p><span className="text-gray-500">Runtime:</span> Tauri 2 + React 18 + Zustand</p>
          <p className="text-xs text-gray-500 pt-2 border-t border-slate-700/50 mt-2">
            Generated from a single 490-line .agi file. <a href="https://github.com/Binary-Blender/agicore" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300">Agicore on GitHub</a>
          </p>
        </div>
      </section>
    </div>
