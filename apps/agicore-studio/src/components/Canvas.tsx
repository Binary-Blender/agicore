      ...
      >
        ...
        <MiniMap
          nodeColor={(n) => {
            ...
            try { return getNodeKind(data.nodeType).miniMapHex; }
            catch { return '#52525b'; }
          }}
...
// Render-time accessor for the node's display detail line. Delegates
// to the registry — each kind owns its own detail-rendering function.
function detailFor(n: WorkflowNode): string | undefined {
  try {
    const def = getNodeKind(n.kind);
    return def.detailFor?.(n);
  } catch {
    return undefined;
  }
}
